...
@implementation MCOManOrderSub2VC
...
- (void)viewDidLoad {
    ...
    //注册通知
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(viewExpress:) name:@"viewExpress2" object:nil];
}
//查快递
-(void)viewExpress:(NSNotification *)noti
{
    ...
    NSString *urlstr = [NSString stringWithFormat:@"https://www.kuaidi100.com/chaxun?com=%@&nu=%@",order.ord_expressname,order.ord_expressid];
...
- (void)loadMoreTopics
{
    ...
    NSDictionary *dict =@{@"que":@"1",@"lastime":lastorder.ord_time};
    [mgr POST:@"http://106.14.145.208/ShopMall/BackShopOrders" parameters:dict progress:^(NSProgress * _Nonnull uploadProgress) {
