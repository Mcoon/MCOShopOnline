...
- (void)emojiBtn:(UIButton *)btn {
    ...
    if (self.emojiClick == NO) {
        ...
        [UIView animateWithDuration:0.25 animations:^{
            ...
            self.frame = CGRectMake(0, K_Height - StatusNav_Height - self.backView.height - bottomHeight + 18, K_Width, self.backView.height + bottomHeight);
...
- (void)changeFrame:(CGFloat)height {
    ...
    self.frame = CGRectMake(0, K_Height - StatusNav_Height - self.backView.height - _keyboardHeight +18, K_Width, self.backView.height);
    //改变更多按钮、表情按钮的位置
...
- (void)keyboardHide {
    ...
    [UIView animateWithDuration:0.25 animations:^{
        //设置self的frame到最底部
        self.frame = CGRectMake(0, K_Height - StatusNav_Height+18, K_Width, self.backView.height);
        [self changeTableViewFrame];
    }];
}
...
- (void)keyboardWillShow:(NSNotification *)notification {
    ...
    [UIView animateWithDuration:duration delay:0 options:[notification.userInfo[UIKeyboardAnimationCurveUserInfoKey] integerValue] animations:^{
        self.frame = CGRectMake(0, endFrame.origin.y- StatusNav_Height, K_Width, self.height);
        [self changeTableViewFrame];
    } completion:nil];
}
