...
- (void)viewDidLoad {
    ...
    //通知
   [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(close:) name:@"logSucuess" object:nil];
   [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(registers) name:@"registers" object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(forgets) name:@"forgetsecure" object:nil];
}
...
-(void)registers
{
    MCORegisterMemVC *vc = [[MCORegisterMemVC alloc] init];
    [self presentViewController:vc animated:YES completion:^{
        _leadCons.constant = 0;
    }];
}
