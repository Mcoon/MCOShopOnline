...
- (void)viewDidLoad {
    ...
    //注册通知
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(headerBeginRefreshing) name:@"refresh" object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(viewExpress:) name:@"express" object:nil];
}

-(void)viewExpress:(NSNotification *)noti
{
    MCOSelfOrder *order = noti.userInfo[@"order"];
    if( order.ord_expressname==nil && order.rev_address!=nil)//商家配送
    {
        UIAlertController *alertVC = [UIAlertController alertControllerWithTitle:@"订单为商家配送" message:nil preferredStyle:UIAlertControllerStyleAlert];
        UIAlertAction *action = [UIAlertAction actionWithTitle:@"我知道了" style:UIAlertActionStyleDefault handler:nil];
         [alertVC addAction:action];
        [self presentViewController:alertVC animated:YES completion:nil];
    }
    else if( order.ord_expressname==nil && order.rev_address==nil)//自提
    {
        UIAlertController *alertVC = [UIAlertController alertControllerWithTitle:@"订单为用户门店自提" message:nil preferredStyle:UIAlertControllerStyleAlert];
        UIAlertAction *action = [UIAlertAction actionWithTitle:@"我知道了" style:UIAlertActionStyleDefault handler:nil];
        [alertVC addAction:action];
        [self presentViewController:alertVC animated:YES completion:nil];
    }
    ...
    {
        MCOExpressItem *item =noti.userInfo[@"exp"];
        //浏览器浏览
        MCOWebViewController *webVc = [[MCOWebViewController alloc] init];
        NSString *urlstr = [NSString stringWithFormat:@"https://www.kuaidi100.com/chaxun?com=%@&nu=%@",item.expressName,item.expressId];
        webVc.url = [NSURL URLWithString: [urlstr stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];//处理中文
...
- (void)loadNewTopics
{
    ...
    NSDictionary *dict =@{@"user_phone":[defaults objectForKey:@"user_phone"],@"que":@"1"};
...
- (void)loadMoreTopics
{
    ...
    NSDictionary *dict =@{@"user_phone":[defaults objectForKey:@"user_phone"],@"que":@"1",@"lastime":lastorder.ord_time};
    [mgr POST:@"http://106.14.145.208/ShopMall/BackUserOrders" parameters:dict progress:^(NSProgress * _Nonnull uploadProgress) {
